...
// Visual warning threshold for montagem — no official limit, just a cue for operators
...
// Standard "tie gets same points" ranking. higherBetter=true means bigger value wins (giro).
...
// Tiebreaker: count 1st-place giro finishes (giroPts === 4) across all complete rounds
...
function MonitorView() {
  ...
  // Load existing result and all-round history for current team
  useEffect(() => {
    let cancelled = false;
    (async () => {
  ...
  // Write live status to storage so Telão can show the indicator
  useEffect(() => {
                ...
                return (
                  ...
                  >
                    <td className="px-2 py-1 font-semibold text-gray-700">
                      R{r}
                      {isCurrent && (
                        <span className="ml-1 text-xs" style={{ color: LARANJA }}>
                          ←
                        </span>
                      )}
                    </td>
...
function TelaoView() {
  ...
  // Sort by total; tiebreaker = most 1st-place giro finishes (per regulamento §5)
